...
class Esense_FormInputFonts extends Esense_FormInput {
	/**
	 *
	...
	public function output() {
		...
		$json_data = $this->tpl->esense_get_json('config', 'fonts');
		//
		$output = '';
		...
		foreach ($json_data as $font_family) {
			...
			'.($this->required).' 
			'.($this->visibility).'>
				<option value="normal"'.selected($type, 'normal', false).'>'.esc_attr__('Standard fonts', 'dp-esense').'</option>
				<option value="squirrel"'.selected($type, 'squirrel', false).'>'.esc_attr__('Fonts Squirrel', 'dp-esense').'</option>
				<option value="google"'.selected($type, 'google', false).'>'.esc_attr__('Google Web Fonts', 'dp-esense').'</option>
			</select></p>';
			// normal fonts selector
			$output .= '<p><label>'.esc_attr__('Font family: ', 'dp-esense').'</label><select id="'.($this->tpl->name).'_fonts_normal_'.($font_family->short_name).'" class="dpInput dpSelect" data-name="fonts_normal_'.($font_family->short_name).'" data-family="'.($font_family->short_name).'" data-type="normal"
			'.($this->required).' 
			'.($this->visibility).'>
			...
			$output .= '<p><label>'.esc_attr__('Fonts Squirrel: ', 'dp-esense').'</label><select id="'.($this->tpl->name).'_fonts_squirrel_'.($font_family->short_name).'" class="dpInput dpSelect" data-name="fonts_squirrel_'.($font_family->short_name).'" data-family="'.($font_family->short_name).'" data-type="squirrel"
			'.($this->required).' 
			'.($this->visibility).'>';
				if(count($squirrel_fonts) > 0) { 
					...
					$output .= '<option value="-1" selected="selected">'.esc_attr__('You have no fonts in fonts/ directory', 'dp-esense').'</option>';
				}
			$output .= '</select></p>';
			// google fonts selector
			$output .= '<p>
			<label for="'.($this->tpl->name).'_fonts_google_'.($font_family->short_name).'">
				'.esc_attr__('Google Web Fonts URL: ', 'dp-esense').'
			</label>
			<input 
			...
			$output .= '<p'.$style.'><label for="'.($this->tpl->name).'_fonts_selectors_'.($font_family->short_name).'">
				'.esc_attr__('Selectors:', 'dp-esense').'
				</label>
				<textarea 
