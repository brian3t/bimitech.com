...
class Esense_FormInput {
	// access to the template object
	protected $esense_tpl;
	 ...
	 *
	 **/
	function __construct($esense_tpl, $name, $label, $tooltip = '', $default = '', $class = '', $format = '', $required = false, $visibility = '', $other = null) {
...
class Esense_FormInputTextBlock extends Esense_FormInput {
...
class Esense_FormInputHTML extends Esense_FormInput {
	public function output() {
...
class Esense_FormInputRawText extends Esense_FormInput {
	public function output() {
...
class Esense_FormInputText extends Esense_FormInput {
	public function output() {
		return '<p data-visible="true"><label 
					for="'.($this->tpl->name).'_'.($this->name).'" 
					title="'.($this->tooltip).'" 
...
class Esense_FormInputTextarea extends Esense_FormInput {	
	public function output() {
		return '<p data-visible="true"><label 
					for="'.($this->tpl->name).'_'.($this->name).'" 
					title="'.($this->tooltip).'"
...
class Esense_FormInputSelect extends Esense_FormInput {
	public function output() {
...
class Esense_FormInputSlider extends Esense_FormInput {
	public function output() {
		$output = '<p data-visible="true"><label 
					for="'.($this->tpl->name).'_'.($this->name).'" 
					title="'.($this->tooltip).'" 
...
class Esense_FormInputSwitcher extends Esense_FormInput {
	public function output() {
		$output = '<p data-visible="true"><label 
					for="'.($this->tpl->name).'_'.($this->name).'" 
					title="'.($this->tooltip).'" 
				>'.$this->label.'</label>
					...
					data-name="'.($this->name).'"
				>';
		$output .= '<option value="N"'.selected($this->value, 'N', false).'>'.esc_attr__('Disabled', 'dp-esense').'</option>';
		$output .= '<option value="Y"'.selected($this->value, 'Y', false).'>'.esc_attr__('Enabled', 'dp-esense').'</option>';
...
class Esense_FormInputMedia extends Esense_FormInput {
	public function output() {
		...
		$output = '<p data-visible="true"><label 
			...
			<input 
				...
				data-name="'.($this->name).'"
			/>
			<input id="'.($this->tpl->name).'_'.($this->name).'_button" class="dpMedia" type="button" value="'.esc_attr__('Upload Image', 'dp-esense').'" />
			<input id="'.($this->tpl->name).'_'.($this->name).'_button1" class="dpMedia" type="button" value="'.esc_attr__('Remove Image', 'dp-esense').'" />
			<small>'.esc_attr__('Enter an URL or upload an image.', 'dp-esense').'</small>
...
class Esense_FormInputColor extends Esense_FormInput {
	public function output() {
...
class Esense_FormInputBackground extends Esense_FormInput {
	public function output() {
		$output = '<p data-visible="true"><label 
					for="'.($this->tpl->name).'_'.($this->name).'" 
					title="'.($this->tooltip).'" 
...
class Esense_FormInputTaxonomy extends Esense_FormInput {
	public function output() {
		$output = '<p data-visible="true"><label 
					for="'.($this->tpl->name).'_'.($this->name).'" 
					title="'.($this->tooltip).'" 
				...
				<select 
					...
					data-name="'.($this->name).'"
				>';
				$output .= '<option value="none"'.selected($this->value, 'not', false).'>'.esc_attr__('--Select--', 'dp-esense').'</option>';
				$slug = 'slideshows';
...
class Esense_FormInputWidthHeight extends Esense_FormInput {
	public function getValue($default) {
		// get the option value from database or if it doesn't exist get the default value
		$this->value = array(
