...
class Esense_MenuWalker extends Walker {
...
function widgets_first_dropdown( &$output, $args ) {
            ob_start();
                esense_dynamic_sidebar( MMPM_PREFIX . '_menu_first_widgets_area' );
	...
	function start_el(&$output, $item, $depth = 0, $args = array(),  $current_object_id = 0) {
		...
		if ($depth ==0) $class_names .= ' menutab'.$count. ' root';
		...
		if (!empty($children)) { 
		if ($depth == 0 && get_option($esense_tpl->name . '_use_indicator') == "Y" ) {$item_output .= '<span class="sub-indicator-0"><i class="icon-angle-down"></i></span>';} else 
