...
include_once(get_template_directory() . '/dynamo_framework/form_elements/standard.php');
...
class Esense_WPFormParser {
	 ...
	 *
	 **/
	function __construct($esense_tpl) {
	 ...
	 *
	 **/
	public function esense_generateForm($filename, $string = false) {
		if(!$string) {
			// load file
			$this->loaded_data = $this->tpl->esense_get_json('options', $filename);
		...
		foreach($this->loaded_data as $group) {
			...
			foreach($group->fields as $field) {
				if(in_array($field->type, $standard_fields)) {
					$className = 'Esense_FormInput' . $field->type;
					...
					// load field config
					$file_config = $this->tpl->esense_get_json('form_elements/'.$field->type, 'config', false);
					...
					if((is_array($file_config) && count($file_config) > 0) || is_object($file_config)) {
						...
						if(!class_exists($file_config->class)) {
							...
							include_once(get_template_directory() . '/dynamo_framework/form_elements/').($field->type).'/'.($file_config->php);
						}
						// create the object
						if(class_exists($file_config->class)) {
