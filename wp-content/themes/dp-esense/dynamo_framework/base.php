...
include_once(get_template_directory() . '/dynamo_framework/classes/class.dpobject.php');
// Including file with template browser class
include_once(get_template_directory() . '/dynamo_framework/classes/class.dpbrowser.php');
// Including file with menu walker class
include_once(get_template_directory() . '/dynamo_framework/classes/class.menu.walker.setup.php');
// Including file with menu walker class
include_once(get_template_directory() . '/dynamo_framework/classes/class.menu.walker.edit.php');
// Including file with menu walker class
include_once(get_template_directory() . '/dynamo_framework/classes/class.menu.walker.php');
// Including file with mobile menu walker class
include_once(get_template_directory() . '/dynamo_framework/classes/class.menu.walker.mobile.php');
// Including file with dotnav menu walker class
include_once(get_template_directory() . '/dynamo_framework/classes/class.menu.walker.dotnav.php');
// Including file used for WPML support
if (defined('ICL_SITEPRESS_VERSION')) {
	include_once(get_template_directory() . '/dynamo_framework/wpml.php');
}
...
class Esense_WP {
	...
	private function read_config() {
		...
		$json_data = $this->esense_get_json('config','template');
	 ...
	 **/
	 ...
	 private function add_actions() {
	 	add_action( 'widgets_init', 'esense_widgets_init' );
	 }
	 ...
	 **/
	 ...
	 private function add_features() {
		...
		add_image_size('esense_portfolio-horizontal', 640,480, true);
		add_image_size('esense_portfolio-square', 640, 640, true);
		add_image_size('esense_portfolio-vertical', 640, 850, true);
       ...
       // add support for widgets in Theme Customizer
		...
		add_theme_support( 'title-tag' );
	 	...
	 	$this->register_fonts();
	 	// add metaboxes
	 	// add Esense Shortcode buttons to the TinyMCE editor
	 	//add_action('admin_init', 'esense_add_editor_buttons');
		}	
	 ...
	 private function register_menus() {
	 	...
	 	$json_data = $this->esense_get_json('config','menus');
		...
		if (is_array($json_data) || is_object($json_data))
		{
	 	foreach ($json_data as $menu) {
	 		...
	 		$this->menu[(string)$menu->location] = array(
	 			...
	 			"style" => ((string) $menu->main == 'true') ? get_option($this->name . '_navigation_menu_style_'.(string) $menu->location, 'esense_extra') : 'esense_normal',
	 		);
	 	}
		}
	 }
	 ...
	 private function register_styles() {
	 	// load and parse JSON file.
	 	$json_data = $this->esense_get_json('config','styles');
	 	// init array
	 	$this->styles = array();
	 ...
	 private function register_fonts() {
	 	// load and parse JSON file.
	 	$json_data = $this->esense_get_json('config','fonts');
	 	// init array
	 	$this->fonts = array();
	 	// iterate through all menus in the file
	...
	**/ 
	...
	public function esense_get_json($dir, $filename, $lang = true) {
		...
		if (defined('ICL_SITEPRESS_VERSION')) {
			$lang = apply_filters('esense-get-json',$dir,$lang);
		}
		if(is_dir(get_template_directory().'/dynamo_framework/' . $dir . '/' . $lang)) {
		$path = get_template_directory() . '/dynamo_framework/' . $dir . '/' . $lang . $filename . '.json';
		} else {
		$path = get_template_directory() . '/dynamo_framework/' . $dir . '/en_US/'. $filename . '.json';
		}
		// check if the specified file exists
		if(file_exists($path)) {
			// decode data from the JSON file
			$json_data = json_decode($wp_filesystem->get_contents($path));
			...
			if(function_exists('json_last_error')) {
				...
				switch(json_last_error()) {
					...
					case JSON_ERROR_NONE:
					    // No errors
					    return json_decode($wp_filesystem->get_contents($path));
					break;
				}
			} else {
					    return json_decode($wp_filesystem->get_contents($path));
