...
/**
 * 
 * DP Tabs Widget class
 *
 **/

class DP_Tabs_Widget extends WP_Widget {
	/**
	 *
	 * Constructor
	 *
	...
	function __construct() {
		parent::__construct(
			'widget_dp_tabs', 
			esc_attr__( 'DP Tabs', 'esense-functions' ), 
			array( 
				'classname' => 'widget_dp_tabs', 
				'description' => esc_attr__( 'Use this widget to show tabs created form the selected sidebar', 'esense-functions') 
			)
		);
		
		$this->alt_option_name = 'widget_dp_tabs';
		//
		add_action('wp_enqueue_scripts', array('DP_Tabs_Widget', 'add_scripts'));
	}
	
	static function add_scripts() {
		wp_enqueue_script( 'esense-tabs', get_template_directory_uri().'/js/widgets/tabs.js', array('jquery'), false, true);
	}
	...
	function widget($args, $instance) {		
		...
		if ($sidebar !== '') {
			...
			echo '<div class="esense-tabs" data-event="'.$event.'" data-autoanim="'.$autoanim.'" data-speed="'.$anim_speed.'" data-interval="'.$anim_interval.'">';
			echo '<div class="esense-tabs-wrap">';
			...
			foreach($widget_code as $code) {
				...
				if(count($title_match) > 1 && isset($title_match[1][0])) {
					...
					array_push($tabs, _e('No title specified!', 'esense-functions'));
				}
				
				array_push($tabs_content, preg_replace('@{TABS_TITLE}(.*?){TABS_TITLE}@mis', '', $code));
			}
			// generate the tabs content
			echo '<ol class="esense-tabs-nav">';
			for($i = 0; $i < count($tabs); $i++) {
				echo '<li'.(($i == 0) ? ' class="active"' : '').'>' . $tabs[$i] . '</li>';
			}
			echo '</ol>';
			
			echo '<div class="esense-tabs-container">';
			for($i = 0; $i < count($tabs_content); $i++) {
				echo '<div class="esense-tabs-item'.(($i == 0) ? ' active' : '').'">' . $tabs_content[$i] . '</div>';
			}
			echo '</div>';
	...
	function update( $new_instance, $old_instance ) {
		...
		if(isset($alloptions['widget_dp_tabs'])) {
			delete_option( 'widget_dp_tabs' );
		}
	...
	function refresh_cache() {
		wp_cache_delete( 'widget_dp_tabs', 'widget' );
	}
	...
	?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'sidebar' ) ); ?>"><?php _e( 'Tabs source:', 'esense-functions' ); ?></label>
			...
			<select id="<?php echo esc_attr( $this->get_field_id( 'sidebar' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'sidebar' ) ); ?>">
				...
				<option value="<?php echo esc_attr($sidebar); ?>"<?php echo (esc_attr($sidebar) == $selected_sidebar) ? ' selected="selected"' : ''; ?>>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('event')); ?>"><?php _e('Tabs activator event:', 'esense-functions'); ?></label>
			...
			<select id="<?php echo esc_attr( $this->get_field_id('event')); ?>" name="<?php echo esc_attr( $this->get_field_name('event')); ?>">
				<option value="click"<?php echo (esc_attr($event) == 'click') ? ' selected="selected"' : ''; ?>>
					<?php _e('Click', 'esense-functions'); ?>
				</option>
				<option value="hover"<?php echo (esc_attr($event) == 'hover') ? ' selected="selected"' : ''; ?>>
					<?php _e('Hover', 'esense-functions'); ?>
				</option>
			</select>
		</p>
		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('autoanim')); ?>"><?php _e('Auto animation:', 'esense-functions'); ?></label>
			...
			<select id="<?php echo esc_attr( $this->get_field_id('autoanim')); ?>" name="<?php echo esc_attr( $this->get_field_name('autoanim')); ?>">
				<option value="enabled"<?php echo (esc_attr($autoanim) == 'enabled') ? ' selected="selected"' : ''; ?>>
					<?php _e('Enabled', 'esense-functions'); ?>
				</option>
				<option value="disabled"<?php echo (esc_attr($autoanim) == 'disabled') ? ' selected="selected"' : ''; ?>>
					<?php _e('Disabled', 'esense-functions'); ?>
				</option>
			</select>
		</p>
		
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'anim_speed' ) ); ?>"><?php _e( 'Animation speed (ms):', 'esense-functions' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'anim_interval' ) ); ?>"><?php _e( 'Animation interval (ms):', 'esense-functions' ); ?></label>
