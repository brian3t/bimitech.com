...
class DP_Flickr_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			esc_attr__( 'DP Flickr Gallery', 'esense-functions' ), 
			array( 
				'classname' => 'widget_dp_flickr_gallery', 
				'description' => esc_attr__( 'Use this widget to show gallery from Flickr', 'esense-functions') 
			)
		);
	...
	?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'flickr_id' ) ); ?>"><?php _e( 'Flickr user ID:', 'esense-functions' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'flickr_limit' ) ); ?>"><?php _e( 'Image count limit:', 'esense-functions' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'flickr_size' ) ); ?>"><?php _e( 'Image size (px):', 'esense-functions' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'flickr_title' ) ); ?>" class="label100"><?php _e( 'Show image title:', 'esense-functions' ); ?></label>
			
			<select id="<?php echo esc_attr( $this->get_field_id( 'flickr_title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'flickr_title' ) ); ?>">
				<option value="false" <?php if(esc_attr( $flickr_title ) == 'false') : ?>selected="selected"<?php endif; ?>><?php _e('Disabled', 'esense-functions'); ?></option>
				<option value="true" <?php if(esc_attr( $flickr_title ) == 'true') : ?>selected="selected"<?php endif; ?>><?php _e('Enabled', 'esense-functions'); ?></option>
			</select>
		</p>
		

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'flickr_link' ) ); ?>" class="label100"><?php _e( 'Show link to flickr stream:', 'esense-functions' ); ?></label>
			
			<select id="<?php echo esc_attr( $this->get_field_id( 'flickr_link' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'flickr_link' ) ); ?>">
				<option value="false" <?php if(esc_attr( $flickr_link ) == 'false') : ?>selected="selected"<?php endif; ?>><?php _e('Disabled', 'esense-functions'); ?></option>
				<option value="true" <?php if(esc_attr( $flickr_link ) == 'true') : ?>selected="selected"<?php endif; ?>><?php _e('Enabled', 'esense-functions'); ?></option>
			</select>
		</p>
