...
class DP_NewsFlash_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'widget_dp_newsflash', 
			esc_attr__( 'DP NewsFlash', 'esense-functions' ), 
			array( 
				'classname' => 'widget_dp_newsflash', 
				'description' => esc_attr__( 'Use this widget to show news flash', 'esense-functions') 
			)
		);
	...
	function widget($args, $instance) {
		...
		echo "   });"."\n";
		echo "</script>"."\n";
		echo '<div id="esense-newsflash" class="'.$this->config['style'].'">';
		...
		echo '<a href="'.get_permalink().'">';
		    if ($this->config['usetitle'] == 'title') {
		    	...
		    	if($this->config['content_type'] == 'content') :
			        echo $this->prepareEsenseContent(get_the_content(false), $this->config['preview_count']).'&hellip;';
		    	else :
		    		echo $this->prepareEsenseContent(get_the_excerpt(), $this->config['preview_count']).'&hellip;';
		    	endif;
		    }
  		 echo '</a>';
		echo '</li>';
    ...
    function prepareEsenseContent($text, $length = 200) {
	...
	?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'esense-functions' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>"><?php _e('Style:', 'esense-functions'); ?></label>
            <select id="<?php echo esc_attr( $this->get_field_id('style')); ?>" name="<?php echo esc_attr( $this->get_field_name('style')); ?>">
				<option value="light"<?php echo (esc_attr($style) == 'light') ? ' selected="selected"' : ''; ?>>
					<?php _e('Light', 'esense-functions'); ?>
				</option>
				<option value="dark"<?php echo (esc_attr($style) == 'dark') ? ' selected="selected"' : ''; ?>>
					<?php _e('Dark', 'esense-functions'); ?>
				</option>
			</select>
		</p>
	    <p>
		    <label for="<?php echo esc_attr( $this->get_field_id( 'catid' ) ); ?>"><?php _e('Posts Category:', 'esense-functions'); ?></label>
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id('catid')); ?>" name="<?php echo esc_attr( $this->get_field_name('catid')); ?>">
					<?php foreach ($categories as $cat) { ?>
					<option value="<?php echo esc_attr($cat->slug); ?>"<?php if($instance['catid'] == $cat->slug) : echo ' selected="selected"'; endif; ?>><?php echo esc_attr($cat->name); ?></option>
					<?php } ?>
			</select>
		</p>
         <p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'article_count' ) ); ?>"><?php _e( 'Max Number of Posts:', 'esense-functions' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('itemsOrdering')); ?>"><?php _e('Order:', 'esense-functions'); ?></label>
		    <select class="widefat" id="<?php echo esc_attr($this->get_field_id('itemsOrdering')); ?>" name="<?php echo esc_attr($this->get_field_name('itemsOrdering')); ?>">
		      	<option value="author"<?php selected($instance['itemsOrdering'], 'author'); ?>><?php _e('Author', 'esense-functions'); ?></option>
		      	<option value="date"<?php selected($instance['itemsOrdering'], 'date'); ?>><?php _e('Date', 'esense-functions'); ?></option>
		      	<option value="title"<?php selected($instance['itemsOrdering'], 'title'); ?>><?php _e('Title', 'esense-functions'); ?></option>
		      	<option value="modified"<?php selected($instance['itemsOrdering'], 'modified'); ?>><?php _e('Modified', 'esense-functions'); ?></option>
		      	<option value="menu_order"<?php selected($instance['itemsOrdering'], 'menu_order'); ?>><?php _e('Menu Order', 'esense-functions'); ?></option>
		      	<option value="parent"<?php selected($instance['itemsOrdering'], 'parent'); ?>><?php _e('Parent', 'esense-functions'); ?></option>
		      	<option value="id"<?php selected($instance['itemsOrdering'], 'id'); ?>>ID</option>
		     </select>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'usetitle' ) ); ?>"><?php _e('Use Title or Content:', 'esense-functions'); ?></label>
            <select id="<?php echo esc_attr( $this->get_field_id('usetitle')); ?>" name="<?php echo esc_attr( $this->get_field_name('usetitle')); ?>">
				<option value="title"<?php echo (esc_attr($usetitle) == 'title') ? ' selected="selected"' : ''; ?>>
					<?php _e('Title', 'esense-functions'); ?>
				</option>
				<option value="content"<?php echo (esc_attr($usetitle) == 'content') ? ' selected="selected"' : ''; ?>>
					<?php _e('Content', 'esense-functions'); ?>
				</option>
			</select>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'content_type' ) ); ?>"><?php _e('Content Type:', 'esense-functions'); ?></label>
            <select id="<?php echo esc_attr( $this->get_field_id('content_type')); ?>" name="<?php echo esc_attr( $this->get_field_name('content_type')); ?>">
				<option value="excerpt"<?php echo (esc_attr($content_type) == 'excerpt') ? ' selected="selected"' : ''; ?>>
					<?php _e('Excerpt', 'esense-functions'); ?>
				</option>
         ...
         <p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'pretext' ) ); ?>"><?php _e( 'PreText Label:', 'esense-functions' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'controls' ) ); ?>"><?php _e('Show Controls:', 'esense-functions'); ?></label>
            <select id="<?php echo esc_attr( $this->get_field_id('controls')); ?>" name="<?php echo esc_attr( $this->get_field_name('controls')); ?>">
				<option value="true"<?php echo (esc_attr($controls) == 'true') ? ' selected="selected"' : ''; ?>>
					<?php _e('On', 'esense-functions'); ?>
				</option>
				<option value="false"<?php echo (esc_attr($controls) == 'false') ? ' selected="selected"' : ''; ?>>
					<?php _e('Off', 'esense-functions'); ?>
				</option>
			</select>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'animation' ) ); ?>"><?php _e('Animation:', 'esense-functions'); ?></label>
            <select id="<?php echo esc_attr( $this->get_field_id('animation')); ?>" name="<?php echo esc_attr( $this->get_field_name('animation')); ?>">
				<option value="slide"<?php echo (esc_attr($animation) == 'slide') ? ' selected="selected"' : ''; ?>>
					<?php _e('Slide', 'esense-functions'); ?>
				</option>
				<option value="fade"<?php echo (esc_attr($content_type) == 'fade') ? ' selected="selected"' : ''; ?>>
					<?php _e('Fade', 'esense-functions'); ?>
				</option>
			</select>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'direction' ) ); ?>"><?php _e('Animation direction:', 'esense-functions'); ?></label>
            <select id="<?php echo esc_attr( $this->get_field_id('direction')); ?>" name="<?php echo esc_attr( $this->get_field_name('direction')); ?>">
				<option value="horizontal"<?php echo (esc_attr($direction) == 'hotizontal') ? ' selected="selected"' : ''; ?>>
					<?php _e('Horizontal', 'esense-functions'); ?>
				</option>
				<option value="vertical"<?php echo (esc_attr($direction) == 'vertical') ? ' selected="selected"' : ''; ?>>
					<?php _e('Vertical', 'esense-functions'); ?>
				</option>
			</select>
		</p>
         <p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'duration' ) ); ?>"><?php _e( 'Transition duration (ms):', 'esense-functions' ); ?></label>
         ...
         <p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'delay' ) ); ?>"><?php _e( 'Transition delay (ms):', 'esense-functions' ); ?></label>
         ...
         <p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'news_indent' ) ); ?>"><?php _e( 'News Indent (px):', 'esense-functions' ); ?></label>
         ...
         <p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'preview_count' ) ); ?>"><?php _e( 'Preview Length (characters):', 'esense-functions' ); ?></label>
