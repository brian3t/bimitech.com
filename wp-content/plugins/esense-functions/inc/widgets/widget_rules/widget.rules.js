...
jQuery(document).click(function (e) {
    if (jQuery(e.target).hasClass('dp_widget_rules_btn') && jQuery(e.target).hasClass('button')) {
        var wrap = jQuery(e.target).next('.dp_widget_rules_wrapper');

        if (wrap.hasClass('active')) {
            wrap.removeClass('active');
            wrap.find('.esense-widget-rules-visibility').val('0');
        } else {
            dp_widget_control_init(wrap);
            wrap.addClass('active');
            wrap.find('.esense-widget-rules-visibility').val('1');
        }
    }
});

// function to init form event
function dp_widget_control_init(selected_form) {
    var form = jQuery(selected_form);    

    if (form && form.attr('data-state') !== 'initialized') {
        form.attr('data-state', 'initialized');
        var firstSelect = form.find('select[name="dp_widget_rules_type"]');
        var select = form.find('.dp_widget_rules_form_select');
        var btn = form.find('.dp_widget_rules_form .dp_widget_rules_btn');
        var form_inputs = {};
        ...
        jQuery.each(form_inputs_names, function (i, el) {
            form_inputs[el] = form.find('.dp_widget_rules_form_input_' + el).parent();
        });
        // hide unnecesary form
        if (firstSelect.children('option:selected').val() === 'all') {
            form.find('.dp_widget_rules_form').css('display', 'none');
        }
        // change event
        firstSelect.change(function () {
            var value = firstSelect.children('option:selected').val();
            var fieldset = form.find('.dp_widget_rules_form');
        ...
        // refresh the list
        dp_widget_control_refresh(form);
        ...
        btn.click(function (event) {
            ...
            var output = form.find('.dp_widget_rules_output');
            var value = select.children('option:selected').val();
            ...
            if (
                ...
                output.val(output.val() + ',' + value + form.find('.dp_widget_rules_form_input_' + value.replace(':', '')).val());
            } else if (value === 'taxonomy:') {
                var tax = form.find('.dp_widget_rules_form_input_taxonomy').val();
                var term = form.find('.dp_widget_rules_form_input_taxonomy_term').val();
                output.val(output.val() + ',taxonomy:' + tax + ((term !== '') ? ';' + term : ''));
            } else if (value === 'posttype:') {
                var type = form.find('.dp_widget_rules_form_input_posttype').val();
                //
                if (type !== '') {
        ...
        form.find('.dp_widget_rules_pages div').click(function (event) {
...
function dp_widget_control_refresh(form) {
    var output = form.find('.dp_widget_rules_output');
    if (output.length > 0) {
        var list = form.find('.dp_widget_rules_pages div');
        list.html('');
        ...
        form.find('.dp_widget_rules_nopages').css('display', pages_exist ? 'none' : 'block');
    }
}
