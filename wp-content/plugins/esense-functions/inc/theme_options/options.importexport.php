...
ob_start();	
function esense_importexport_options() {
...
?>
	<div class="dpWrap wrap">
		<h1><big><?php echo esc_html($esense_tpl->full_name); ?></big><small><?php _e('Based on the Dynamo WP framework', 'esense-functions'); ?></small></h1>
		<div class="dpImport">
			<h2>Import Template Settings</h2>
			<?php
				if (isset($_FILES['import']) && check_admin_referer('esense_importexport')) {
		...
		?>
			...
			<form method='post' enctype='multipart/form-data'>
				<p class="submit">
					<?php wp_nonce_field('esense_importexport'); ?>
...
<?php 
  	} elseif (check_admin_referer('esense_importexport')) {
		$option_prefix = $esense_tpl->name;
		$blogname = str_replace(" ", "", get_option('blogname'));
		$date = date("d_m_Y_H_i_s");
		$json_name = $blogname."_".$esense_tpl->name."_".$date; // Generating filename.
   ...
   /**
   	...
   	**/ 
   	...
   	function loadWidgetSettings() {
   		...
   		update_option($esense_tpl->name . '_widget_settings_loaded', 'Y');
